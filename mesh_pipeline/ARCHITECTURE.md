...
| p5_mouth | `skipped` (bool), `mode` (`"carved_closed"` \| `"reference_kept"` \| `null`), `teeth_recess_mm` (float, null in `reference_kept` mode), `opening_z_range` ([lo,hi], thin closed-lips slit at the outer skin surface, measured after zip-close), `bag_z_range` ([lo,hi], interior cavity where teeth/tongue live, null if unmeasurable), `teeth_z_range` ([lo,hi]), `lip_gap_mm_measured` (float, mm, measured after zip-close), `teeth_reference_kept` (bool), `lips_topologically_split` (bool), `rim_loop_count` (int, connected rim loops measured right after the boolean cut and before zip-close; a clean cut is 1, up to 2 tolerated, `> 2` stops the phase before zip-close/inserts with `needs_review`), `inserts_outside_fraction` (float 0..1, max across teeth/tongue objects of the fraction of up to 50 sampled verts that land outside the body per the docs Phase 7 BVH `find_nearest` test; `> 0.05` for any insert sets `needs_review`) |
